...
(function () {
  ...
  function writeHash(groups) {
    ...
    groups.forEach(function (g) {
      ...
      const checked = g.pills.filter(function (p) {
        return p.input.checked;
      }).length;
      ...
      parts.push(
        HASH_KEYS[g.key] + '=' + vals.map(encodeURIComponent).join(','),
      );
    });
  ...
  function collectGroups(barEl) {
    ...
    barEl.querySelectorAll('.filter-pills').forEach(function (container) {
      ...
      const inputs = Array.from(
        ...
        return !i.hasAttribute('data-huc6');
      });
  ...
  function wireHucHierarchy(barEl, ctx) {
    ...
    function refreshParent(pair) {
      const checked = pair.children.filter(function (x) {
        return x.checked;
      }).length;
  ...
  function matches(row, groups) {
    for (let i = 0; i < groups.length; i++) {
      ...
      for (let j = 0; j < g.pills.length; j++) {
        ...
        if (values.indexOf(p.input.value) !== -1) {
          anyChecked = true;
          break;
        }
      }
  ...
  function refilter(ctx) {
    ...
    if (ctx.countEl) {
      ctx.countEl.textContent =
        visible + (visible === 1 ? ' reach' : ' reaches');
    }
  ...
  function wireAllNone(group, ctx, afterToggle) {
    ...
    container.addEventListener('click', function (e) {
      ...
      if (t.matches('[data-all]')) {
        e.preventDefault();
        group.pills.forEach(function (p) {
  ...
  function init(opts) {
    ...
    if (!bar) return null;
    const tbody =
      opts?.rowsContainer || document.querySelector('table.levels tbody');
    if (!tbody) return null;

    const rows = Array.from(
      tbody.querySelectorAll(
        'tr[data-state], tr[data-basin], tr[data-huc8], tr[data-status], tr[data-tier]',
      ),
    );
    ...
    if (window.matchMedia?.(DESKTOP_QUERY).matches) {
      bar.querySelectorAll('details.filter-group').forEach(function (d) {
        d.open = true;
      });
    }
