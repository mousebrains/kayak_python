...
Subcommands (Tier 4 of docs/PLAN_editor_pr_bridge.md):
  * ``status``   — show the bridge queue counts by state (read-only).
  * ``run-once`` — turn queued endorsements into kayak_data PRs (the worker).

The ``queue`` / ``reconcile`` / ``mark-deployed`` subcommands (manual requeue +
the Tier 5 deploy-closure loop) land in a follow-up. ``run-once`` is gated on
``editor_bridge_enabled`` + App credentials, so a host without the GitHub App
provisioned is a clean no-op rather than an error.
"""
