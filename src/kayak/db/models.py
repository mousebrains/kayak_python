...
class FetchUrl(Base):
    ...
    Rows arrive in the DB only via ``levels sync-metadata`` from the dataset's
    ``fetch_url.csv`` (generated by ``levels generate-sources`` from the
    dataset's ``sources.yaml``) — ``levels init-db`` is schema-only since the
    S1-cleanup, so a fresh DB has none until the first sync. The ``parser`` field
