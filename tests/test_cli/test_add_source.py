...
`add-source` appends a validated source (bound to an existing gauge via
``--gauge-id``, plus an optional new fetch_url) to a dataset's `sources.yaml`,
allocates its stable id(s) from `id_counters.csv`, bumps the counter(s), and
regenerates source.csv + fetch_url.csv + gauge_source.csv — atomically, preserving
the byte round-trip invariant. These exercise the three source shapes, id
allocation, the add-source-specific guards (incl. the mandatory gauge link), usage
errors, atomic failure isolation, and the reverse_engineer serializer.
"""
...
FIXTURE = Path(__file__).resolve().parents[1] / "fixtures" / "dataset"  # gauges 1,2,3
...
def _snapshot(d: Path) -> dict[str, bytes]:
    files = ("sources.yaml", "id_counters.csv", "source.csv", "fetch_url.csv", "gauge_source.csv")
    return {n: (d / n).read_bytes() for n in files if (d / n).is_file()}


def _gauges(d: Path, *ids: int) -> None:
...
def test_add_fetch_backed_source(dataset: Path) -> None:
    ...
    # the mandatory gauge link is materialized as a gauge_source row
    gs_pairs = {(r["gauge_id"], r["source_id"]) for r in _rows(dataset, "gauge_source")}
    assert ("1", "4") in gs_pairs
...
def test_add_calc_backed_source(dataset: Path) -> None:
    allocated = gs.add_source(
        dataset, name="NewCalc", gauge_id=2, agency="Calculation", calc_expression_id=1
    )
    ...
    assert ("2", "4") in {(r["gauge_id"], r["source_id"]) for r in _rows(dataset, "gauge_source")}


def test_add_detached_usgs_ogc_source(dataset: Path) -> None:
    allocated = gs.add_source(dataset, name="12345678", gauge_id=3, agency="USGS")
    assert allocated == {"source": 4}
    src = {r["name"]: r for r in _rows(dataset, "source")}["12345678"]
    assert src["fetch_url_id"] == "" and src["calc_expression_id"] == ""
    assert ("3", "4") in {(r["gauge_id"], r["source_id"]) for r in _rows(dataset, "gauge_source")}


def test_disabled_fetch_url(dataset: Path) -> None:
    gs.add_source(
        dataset, name="OFFW1", gauge_id=1, url="https://example/off", parser="nwps", enabled=False
    )
...
def test_counter_row_order_preserved(dataset: Path) -> None:
    ...
    gs.add_source(dataset, name="NEWW1", gauge_id=1, url="https://example/x", parser="nwps")
...
def test_duplicate_name_rejected(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match="already exists"):
        gs.add_source(dataset, name="FXTW1", gauge_id=1)  # FXTW1 is in the fixture
...
def test_duplicate_url_rejected(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"url .* already exists"):
        gs.add_source(dataset, name="DUPW1", gauge_id=1, url=existing, parser="nwps")
    assert _snapshot(dataset) == snap


def test_unknown_gauge_id_rejected(dataset: Path) -> None:
    snap = _snapshot(dataset)
    with pytest.raises(ValueError, match=r"gauge_id 999 not in gauge\.csv"):
        gs.add_source(dataset, name="NOGAUGE", gauge_id=999)
    assert _snapshot(dataset) == snap
...
def test_calc_id_must_exist(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"not in calc_expression\.csv"):
        gs.add_source(dataset, name="BadCalc", gauge_id=1, calc_expression_id=42)
    assert _snapshot(dataset) == snap
...
def test_calc_id_rejected_when_no_calc_csv(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"calc_expression\.csv not present"):
        gs.add_source(dataset, name="BadCalc", gauge_id=1, calc_expression_id=1)
...
def test_missing_counter_row_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="no row for table 'source'"):
        gs.add_source(d, name="X", gauge_id=1)
...
def test_malformed_current_registry_falsy_section_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"current sources\.yaml is invalid"):
        gs.add_source(d, name="NEW", gauge_id=1, agency="USGS")
...
def test_malformed_current_registry_truthy_section_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match=r"current sources\.yaml is invalid"):
        gs.add_source(d, name="NEW", gauge_id=1)
...
def test_usgs_numeric_name_accepted(dataset: Path) -> None:
    gs.add_source(
        dataset, name="14998000", gauge_id=1, agency="USGS", url="https://example/u", parser="nwps"
    )
...
def test_url_without_parser_library_guard(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match="requires a parser"):
        gs.add_source(dataset, name="A1", gauge_id=1, url="https://example/x")
...
def test_malformed_calc_csv_clean_error(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match="non-integer id"):
        gs.add_source(dataset, name="A1", gauge_id=1, calc_expression_id=1)
...
def test_non_positive_next_id_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="must be >= 1"):
        gs.add_source(d, name="NEGW1", gauge_id=1)
...
def test_validation_failure_leaves_files_untouched(dataset: Path) -> None:
    ...
    with pytest.raises(ValueError, match="not a valid IANA timezone"):
        gs.add_source(dataset, name="TZW1", gauge_id=1, timezone="Mars/Phobos")
    assert _snapshot(dataset) == snap
...
def test_round_trip_holds_after_add(dataset: Path) -> None:
    gs.add_source(dataset, name="NEWW1", gauge_id=1, url="https://example/rt", parser="nwps")
...
def test_cli_happy_path_returns_zero(dataset: Path) -> None:
    ns = _ns_add(dataset, name="CLIW1", gauge_id=2, url="https://example/cli", parser="nwps")
...
# --- reverse_engineer serializer: byte-identical output -----------------------
...
def test_reverse_engineer_output_unchanged(dataset: Path) -> None:
    ...
    # serializer (incl. always-emit-agency + the required gauge_id) must reproduce
    # it byte-for-byte.
