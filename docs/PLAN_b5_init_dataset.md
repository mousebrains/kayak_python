...
| 9 | No WKCC/Oregon assumptions in site output except dataset-supplied | **Verified** | wheel-smoke "regional neutrality" check: a fixture-dataset build carries no `wkcc`/`willamette` tokens. (The former `status.php` CORS exclusion is gone — it now reads `HostConfig.allowed_origins`, so the check runs with no exclusion.) |
