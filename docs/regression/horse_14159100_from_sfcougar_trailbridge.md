...
**Caveat 1 (autocorrelation)**: this is the **OLS** covariance, which assumes IID residuals; with lag-1 residual autocorrelation **0.82** it understates the parameter SE by roughly **9.3x**. Use the block-bootstrap SEs/CIs in the coefficients table for inference, not these (monthly blocks; longer blocks would only widen the intervals, so they are conservative for the most autocorrelated fits).
...
Inter-gauge travel-time structure from USGS unit values (30-min grid, 45,302 points); full analysis in [`horse_14159100_leadlag.md`](./horse_14159100_leadlag.md). The daily coefficients above are applied in production to *instantaneous* readings, so these lags are the timing error a correction would address. **+τ** = upstream (a past read, deployable in real time); **-τ** = downstream (a future read — non-causal look-ahead).

| Predictor | applied τ (h) | Δ-corr | direction |
|---|---|---|---|
| 14159200 `14159200` | +0.0 | 0.395 | co-located |
| McKenzie at Trail Bridge Dam (upstream, dominant) `14158850` | +0.0 | 0.140 | held (not identifiable) |

**Full** alignment (incl. downstream → future): +0.0% RMSE, 95% CI [+0.00, +0.00] cfs (CI through 0). **Deployable** (causal, upstream-only): +0.0%, [+0.00, +0.00] cfs (CI through 0). **Verdict: no resolvable sub-daily lag** — keep using contemporaneous readings.
