# Sub-daily lead/lag: USGS 14159100 regression

Companion to [`gauge_pair_linear.py`](../../scripts/regression/gauge_pair_linear.py) and the daily-mean fit in [`horse_14159100_from_sfcougar_trailbridge.md`](./horse_14159100_from_sfcougar_trailbridge.md). **Question (informational):** the daily-mean fit averages away the sub-daily travel time between gauges — how large is that timing structure, and how much of it is real-time-usable?

![CCF vs lag](./horse_14159100_leadlag.svg)
...
python3 scripts/regression/gauge_lead_lag.py \
    ...
    --end 2026-04-02 \
    --grid-minutes 30 \
    --name horse_14159100_leadlag
```

## Data

USGS **unit values** resampled to a common **30-min** UTC grid over **2023-08-24 → 2026-04-02**. Overlap where the target and all 2 predictors have a value: **45,302 points** (~2.6 years). Each gauge uses discharge where available, else gage height (timing is identical — USGS derives flow from stage instantaneously):

| Role | Gauge | Label | variable |
|---|---|---|---|
| target | `14159100` |  | flow |
| predictor | `14159200` |  | flow |
...
|---|---|---|---|---|
| 14159200 `14159200` | +0.0 | 0.395 | **+0.0** | co-located / sub-grid travel |
| McKenzie at Trail Bridge Dam (upstream, dominant) `14158850` | -1.0 | 0.140 | **+0.0** | not identifiable (peak Δ-corr 0.14); held contemporaneous |
...
|---|---|---|---|---|
| daily-trained | contemporaneous | 45,302 | 0.9170 | 109.7 |
| daily-trained | full (incl. downstream) | 45,302 | 0.9170 | 109.7 |
| daily-trained | deployable (upstream-only) | 45,302 | 0.9170 | 109.7 |
| hourly-refit | contemporaneous | 45,302 | 0.9624 | 73.8 |
| hourly-refit | full (incl. downstream) | 45,302 | 0.9624 | 73.8 |

Daily-mean reference (same 2 predictors, 1962-10-01→2026-04-01, n=3,508): RMSE **88.4 cfs**, r² 0.9354 — daily means are smoother than instantaneous values, so this sits below the grid RMSEs and isn't directly comparable to them.

### Is the gain statistically real, and is it usable?

Grid residuals are strongly autocorrelated (lag-1 **0.99**), so the 45,302 points carry far fewer independent observations than their count. A **block bootstrap** over 7-day blocks (137 of them, B=2000) on the RMSE reduction (contemporaneous minus aligned):

| Alignment | gain | mean Δ (cfs) | 95% CI (cfs) | better in | resolved? |
|---|---|---|---|---|---|
| **full** (incl. downstream future) | +0.0% | +0.00 | [+0.00, +0.00] | 0% | no (CI ∋ 0) |
| **deployable** (causal, upstream) | +0.0% | +0.00 | [+0.00, +0.00] | 0% | no (CI ∋ 0) |

During the **fastest-changing 10% of points** (|Δtarget| ≥ 16 cfs/30min, n=4,553), where misalignment should bite hardest, full alignment changes RMSE by **+0.0%** (228.4 → 228.4 cfs).

## Verdict

**No resolvable sub-daily lag.** Every predictor is co-located within the 30-min grid or below the identifiability floor (0.15), so there is nothing to align — full and deployable alignment are both identical to contemporaneous. **Keep contemporaneous readings.**
...
- **Caveat:** the grid hold-out (2023-08-24..2026-04-02, ~2.6 yr) is far shorter than the daily fit's record; the daily-reference row controls for the predictor-set change, not the window.
