...
This plan closes the lint coverage gap (Phase 1, required) and decides on a `var → const/let` modernization pass (Phase 2, optional, scoped separately for clean rollback).

## Why

Biome's lint gate only works for files it lints. Four of the eleven hand-written JS files (`gauge_picker.js`, `feature-map.js`, `plot-hover.js`, `filters.js`) are live — referenced from PHP and the build pipeline — but invisible to CI. The exclusion looks accidental: `picker.js` is in the includes but its sibling `gauge_picker.js` is not; `levels.js` is in but its sibling `filters.js` is not.

Past that, the codebase is split between modern JS (`picker.js`, `gauge_picker.js`, `sw.js`: 0 `var`s total) and pre-`let`/`const` JS (7 files with 236 `var`s total — biggest offenders: `map.js` 73, `plot-hover.js` 50, `filters.js` 43, `feature-map.js` 31). Whether to harmonize is a separate, lower-stakes call.

Goal: every hand-written JS file is under CI lint; biome reports 0 warnings; a documented decision on `var` modernization.
...
Note also: `plot-hover.js` is the *only* JS file in the project that uses `'use strict'`. The other ten lack it — they're already non-strict by default. So the autofix removes an inconsistency at the cost of strict-mode safety in one file. Phase 1 step 3 assumes the recommended path (suppress the rule, keep the directive); see Decision 1 for the alternative.
...
## Phase 2 — `var → const/let` modernization (4 commits, optional)

**Decision deferred — see *Decisions to make* below.** If you decide to do it:

Scope is **236 `var` declarations across 7 files**:

| File | `var` count | Phase 2 commit |
|---|---|---|
| `static/map.js` | 73 | 2a (biggest, riskiest first) |
| `static/plot-hover.js` | 50 | 2a |
| `src/kayak/web/static/filters.js` | 43 | 2b |
| `static/feature-map.js` | 31 | 2b |
| `static/reach-map.js` | 19 | 2c (small tail, bundled) |
| `static/search-map.js` | 13 | 2c |
| `src/kayak/web/static/levels.js` | 7 | 2c |
| **+ rule flip** | — | 2d |
...
1. **2a, 2b, 2c — per-file modernization.** Enable `noVar: "warn"` temporarily, run `biome check --write --unsafe` on the file batch, review the resulting `var → let` diff for any reassignment-across-closures edge cases, then run `useConst` (also enabled temporarily) to upgrade non-reassigned `let` to `const`. Stage and commit per file batch.
2. **2d — rule flip.** Once all files are clean: flip `noVar` and `useConst` to `"error"` in `biome.json`'s rule block, remove the temporary `"useConst": "off"` line. One-line commits prevent future regressions.

Splitting into 4 commits (vs one big diff) keeps each batch reviewable in isolation and lets browser smoke-tests narrow the blast radius if a refactor introduces a real regression.
...
**Verification gate:** Phase 1 gate plus per-commit browser smoke-test of the file's primary use case (e.g. for the 2a commit covering `map.js`, walk through every map interaction on `map.html`). Side-by-side compare against `main` in a second tab — the modernized version must show identical behavior, zero console errors. This is harder than Phase 1's gate because the changes are more numerous and the autofix is less mechanical at the semantic level.

## Decisions to make

1. **`noRedundantUseStrict` — suppress or apply autofix?** Biome's rule misfires for classic scripts (the project's actual loading mode — every `.js` file is loaded via `<script src="…" defer>`, none via `type="module"`):
   - **Suppress rule** in `biome.json` (`"suspicious": { "noRedundantUseStrict": "off" }`): preserves plot-hover.js's strict-mode safety. Documents that biome's module assumption is wrong for this project. Future classic-script files can adopt `'use strict'` without lint noise. **Recommended; Phase 1 step 3 assumes this path.**
   - **Apply autofix** (remove `'use strict'` from `plot-hover.js:30`): aligns plot-hover.js with the other 10 JS files (none of which use the directive). Cosmetic consistency at the cost of strict-mode safety in one file. Future `'use strict'` additions to other classic-script files would re-trigger the misfire. If you pick this path, drop step 3's suppression and apply the fix in step 4's `--unsafe` batch.

2. **Do Phase 2 now, defer it, or skip it?** The work is mechanical but the verification gate (browser smoke-test of every JS-driven flow) is the labor-intensive part. Skip-or-defer reasoning: no current bug points at `var` semantics; CI doesn't fail on it; no other code in this project blocks on it. Do-it reasoning: while you're already in the JS files for Phase 1, the marginal cost of also modernizing them is low, and `useConst` enabled-everywhere stops the slow style drift. **Recommend deferring** unless you're already touching those files for an unrelated bug — the verification gate is the bottleneck and the safety win is small.

3. **If skipping Phase 2, leave `useConst` `"off"` or remove the entry?** Leaving `"off"` documents the conscious choice. Removing it lets biome's default (`recommended: true`) kick in next time biome upgrades and re-introduce the noise. Recommend: leave `"off"`. (No inline comment possible — biome.json rejects JSON comments. Rationale stays in this plan + git history.)
...
- **The 4 files have no JS unit tests.** Neither does any other JS in the repo. Phase 1's auto-fixes are mechanical enough that a one-line code-review pass + browser smoke-test is the right level of safety. Phase 2 doesn't have that same easy-review property; the verification gate is the bottleneck.
