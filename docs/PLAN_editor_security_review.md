...
1. **Phase 0.1 — Surface inventory.** `docs/security/editor-surface.md`:
   ...
   - Every DB table with PII or auth material: `editor` (email, status), `editor_session` (token_hash, ip, user_agent, last_seen_at, revoked_at), `editor_magic_link` (token_hash, ip_issued, expires_at, used_at), `maintainer_credential` (WebAuthn credential blob — schema only, not yet wired), `change_request` (proposer + body), `change_request_attachment` (uploads — not yet wired), `edit_history` (changed_by, old/new value)
