...
- **Existing controls audit (preview — full sweep is Tier 0.4):** session cookie attributes are already strong (`HttpOnly`, `SameSite=Strict`, `Secure` when HTTPS, per `php/includes/auth.php:_cookie_params`); CSRF uses double-submit cookie with `hash_equals` constant-time compare (`csrf_token` + `require_csrf` in same file); magic-link tokens are 256-bit `random_bytes(32)` stored as sha256 hash; brute-force protection has THREE layers (nginx `limit_req`, fail2ban, Turnstile — see Phase 1.4); a privacy policy already exists at `php/privacy.php`; a `security.txt` already exists at `static/security.txt`. Tier 0.4 should confirm these and look for gaps.
...
4. **Phase 1.4 — Brute-force / credential-stuffing posture.** **Three layers already in place** (Tier 0.4 will mark all three ✓; this phase audits gaps):
   - **nginx `limit_req`** zones in `deploy/ratelimit.conf`: `login:1m@3r/min`, `auth:1m@10r/min`, `edit:1m@5r/min`, `contact:1m@10r/min`, `php:10m@5r/sec`, `global:10m@20r/sec`. All key on `$binary_remote_addr` — per-IP only.
   - **fail2ban** jails in `deploy/fail2ban/jail.local` + `jail.d/kayak-edit.conf` + `jail.d/kayak-editor-auth.conf` (filters in `filter.d/nginx-edit-auth.conf`, `nginx-editor-auth.conf`, `nginx-default-block.conf`, `nginx-malicious.conf`). 1h ban with `bantime.increment=true` up to 1w. This catches repeat offenders that grind through the limit_req allowance.
   - **Turnstile** on `/login.php` and `/contact.php` (Cloudflare bot challenge).
   - **Application-side `magic_link_under_throttle()`** per-IP + per-email rolling-hour cap (per `tests/php/AuthTest.php`).
   - Audit: confirm fail2ban filter regexes still match the current nginx error-log format (kayak-error.log); confirm `/var/log/nginx/kayak-error.log` is the path the jail watches; assess whether per-IP-only at the limit_req layer is sufficient (a botnet rotating IPs can grind past, but fail2ban catches them at next-tier); decide whether to add per-account lockout (e.g. on the `editor` row) for additional defense.
...
4. **Decision point — privacy policy + terms:** A privacy policy already exists at `php/privacy.php` (last updated 2026-05-01) covering: server access logs (IP, UA, referer), cookies (`ed_sess` + `ed_csrf` with attributes), contributor email retention, proposed edits/comments, no third-party tracking, login email via Google mail relay. **No terms-of-service page yet.** Decision menu becomes:
   - **Keep current privacy.php; add nothing.** Defensible for a hobby/club site; existing coverage is honest and reasonable.
   - **Refresh privacy.php's "Last updated" + audit accuracy.** As the editor feature evolves, the page can drift. Set an annual review trigger.
   - **Add a brief Terms of Service page** (`/terms.php`). Sets expectations about acceptable contributions, content licensing, dispute handling. ~1-2 hours; minimal commitment.
   - **Lawyer-reviewed.** Days of work + cost; appropriate only if revenue or specific external obligations appear.
