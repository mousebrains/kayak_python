...
- **CSP is set in nginx, not PHP.** Per `php/csp-report.php:7`, the nginx vhost emits `Content-Security-Policy` with `report-uri /csp-report.php`. Good news: the PHP split can't accidentally weaken CSP. Risk: an extracted helper could `header('Content-Security-Policy: ...')` and override the nginx-set policy at the response level. The Tier 1.3 integration test should assert presence of the expected CSP header on every response (and absence of any PHP-set CSP header).
...
- **CSP is per-location, not global.** `deploy/levels` sets CSP via per-block `add_header` (browser-cache rules vary per asset type). The "no PHP-set CSP" assertion in Tier 1.3 tests should compare against the expected per-route CSP, not a single value.
