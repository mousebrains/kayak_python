...
Remote URLs to pull observation data from. Rows arrive in the live DB via `levels sync-metadata` from the dataset's `fetch_url.csv` (generated by `levels generate-sources` from the dataset's `sources.yaml`); `levels init-db` also seeds them from the engine-packaged `sources.yaml` for a fresh dev DB. The `parser` field names a registered parser class; `hours` restricts which UTC hours of the day this URL should be fetched (e.g. `"6,12,18"`).
...
| `is_active` | BOOLEAN | Default 0. Only `is_active=1` rows are fetched. `init-db`'s `sync_sources` flips rows to 0 when they disappear from `sources.yaml` |
