...
> - iter 4 (2026-05-12): 4 findings — Playwright's `baseURL` is evaluated at config load (before `globalSetup` runs); `process.env.KAYAK_TEST_BASE_URL` set in `globalSetup` would arrive too late. Replaced the env-var hand-off with a hardcoded `baseURL: 'http://127.0.0.1:8000'` in `playwright.config.ts` (port 8000 is the canonical test port; dev-box conflicts are rare and trivially resolvable). globalSetup no longer needs to export `KAYAK_TEST_BASE_URL`. Phase 3's test-table dropped from 6 to 4 entries after iter 3's `/custom.php`+`/custom_gauges.php` removals; End State updated from "7 smoke tests" to "5 total" (1 in Phase 1 + 4 in Phase 3). Cross-check ref bumped to `5a00969`.
> - iter 5 (2026-05-12): 2 findings — `docs/operations.md` doesn't exist in the repo; End State's "tracked separately as a `docs/operations.md` line" was over-claiming about a hypothetical doc. Removed. Verified `/reach.php?st=OR` against an empty-reach DB returns `'No reaches matching OR.'` (`php/reach.php:193`) which still contains the `'reaches matching'` substring asserted by Phase 1's drill — assertion is robust to data state. Cross-check ref bumped to `cc45d24`.
> - iter 6 (2026-05-12, this revision, stopping): 1 finding — Phase 3 verification gate's "6-7 tests ≈ 5 s sequential" estimate was stale after iter 3 dropped two tests; corrected to "5 tests ≈ 4 s sequential." Convergence: 5 → 4 → 5 → 4 → 2 → 1 findings. Remaining items are aesthetic (e.g. Reproduce-section curl to api.github.com requires internet; left in as advisory).
>
...
This OUTPUT_DIR isolation aligns with `docs/done/PLAN_dev_env_followups.md` Phase 3's convention (the dev box sets `OUTPUT_DIR` outside the repo); the test does the same thing for a tmp dir.
...
- Total test time stays under 60 s on cached runs (each Playwright page-goto is ~500-800 ms; 5 tests ≈ 4 s sequential).
