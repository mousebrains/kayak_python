...
**Batch 4 — S7 + S8 (3 engine PRs). ✓ DONE 2026-06-14/15** (engine #190–#197; see the Eighth pass below + `PLAN_4c_renderers.md`):
...
**Checkpoint 2 — the S7 cutover (the one genuinely risky deploy). ✓ DONE 2026-06-14/15, no downtime.**
...
## Eighth pass — status and remaining (2026-06-16)

Supersedes the seventh-pass schedule above. Cross-checked against `main`
(this doc had been branch-only and many commits behind). It is consolidated
onto `main` by the **Batch-5 PR that carries this edit** — making real the
eighth-pass "consolidated onto main" claim that earlier never landed; the
stale `plan-dataset-separation` branch can then be retired.

### Done since the seventh pass

- **Batch 3 (SA-lite, R5) — MERGED** (engine #188, 2026-06-12). Editor
  approval *endorses* and freezes the validated diff in `applied_json`;
  `edit.php` maintainer edits no longer write `reach`/`gauge`. Criteria 6
  and 7 closed.
- **Checkpoint 1** — folded into the Checkpoint-2 cutover deploy (it ships
  main-tip, so batches 1–3 went live with it).
- **Batch 4 (S7 + S8, R7) — DONE** (engine #190–#197). 4A typed host
  config + renderers; 4B `/opt/kayak` paired releases + `kayak-deploy` +
  maintenance/rollback + clean-VM smoke; 4C runbooks + WKCC migration.
  Backup policy (S8) folded into 4A. Detail: `PLAN_4c_renderers.md`.
- **Checkpoint 2 — the cutover — DONE, no downtime (2026-06-14/15).**
  Live site on the paired-release layout, served from
  `/var/cache/kayak/docroot`; pre-cutover DB snapshot retained; old
  `/home/pat` layout kept dormant for rollback.
- **Post-cutover `public_html` cleanup — DONE (2026-06-16).** Retired the
  pre-cutover `/home/pat/public_html` docroot default (engine #203) and
  repointed the audit probes (kayak_data #62); both deployed, config-drift
  back to clean.

### Remaining

- **Batch 5 — S5 / bootstrap (R8) — IMPLEMENTED in the B5 PR** (this branch;
  see `PLAN_b5_init_dataset.md`). `levels init-dataset` (+ `--example` packaged
  example, `--ci` dataset-repo workflow emission) and the new-region runbook
  (`docs/new-region-runbook.md`) landed; the example dataset now ships in the
  wheel and `scripts/wheel-smoke.sh` exercises `init-dataset --example` from
  site-packages. The acceptance checklist (criteria 1–12) is recorded in
...
- **`analyze-logs` release-time default — FIXED (#205, merged 2026-06-18).**
  ...
  `/opt/kayak/current` release-pointer mtime (the activation instant, correct
  after a rollback), derived from `HostConfig.release_root` — a better signal
  than `docroot`, which the hourly build rewrites.
- **Full SA bridge** (automated approval→PR worker) — deliberately
  deferred under decision D1 (SA-lite shipped); revisit only if editor
  volume grows.
- **Archive on completion.** When batches 5–6 land and the acceptance run
  is recorded, move this doc to `docs/done/` (the convention for finished
  plans).
