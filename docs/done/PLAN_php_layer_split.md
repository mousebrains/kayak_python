...
| `review.php`       | 318 | 28  | 5.R.1 `00641ac` / 5.R.2 `96856df` / 5.R.3 `83ec97c` | ✓ Done — `review_handler.php` (462); reused 5.P.1 editor-session infra |
| `includes/gauge_plots.php` | 386 | 222 | 5.GP.1 `2665a7b` | ✓ Done — split into `gauge_plots_filter.php` (200, pure math) + `gauge_plots_data.php` (107, DB queries); public API unchanged |
| `includes/auth.php`        | 408 | 306 | 5.A.1 `c161ae5` / 5.A.2 this commit | ✓ Done — `auth_magic_link.php` (205, magic-link cluster); session+CSRF+gates stay in auth.php |

**Tier 5 complete — 7 of 7 files done.** Per-file gates (php -l + PHPStan + cs-fixer + integration tests green) met on each commit; CI green. Tier 5 added 4 new handler files (gauge_search/gauge_detail/propose_handler/review_handler), 2 paired entry-point handlers (custom_handler/custom_gauges_handler), and 3 sub-helper files (gauge_plots_filter/data + auth_magic_link), for a total of 9 new files extracted from 7 source files. Combined line shift: source files went from 2,705 lines (388 avg) to 532 lines (76 avg), while extracted helpers totaled 3,189 lines — net +1,016 lines as expected (each extracted body adds a function signature + PHPDoc, but each source file becomes scannable).
...
**5.GP transitive include pattern:** gauge_plots.php's split into filter (pure math) + data (DB) + render (top-level) used a transitive-include trick: gauge_plots.php requires gauge_plots_data.php, which requires gauge_plots_filter.php. Consumers (description_detail.php, gauge_detail.php) still write a single `require_once gauge_plots.php` and get the whole stack. Zero consumer-side edits — same shape as Tier 4's svg_plot_rating.php transitive include. Apply this pattern whenever a helper-split has a clean inner→outer dependency chain.

**5.A bottom-of-file include:** auth.php's split into core (session+CSRF) + auth_magic_link.php used a bottom-of-file include because auth_magic_link.php needs `generate_token` and `hash_token` from auth.php to define itself. Putting the include at the top would create a load-order cycle (auth.php → auth_magic_link.php → auth.php), so the include sits below the token-util definitions. Future auth-feature splits that need auth.php's core helpers should follow the same pattern. Worth noting: PHP's `require_once` is idempotent, so the apparent cycle is safe — the include guard prevents re-execution, and PHP completes the outer file's parse before evaluating the require_once line.

**Tier 5 complete — handing off to Tier 6.** The 7 source files (4 entry points + 3 helper-split targets) are all extracted. Tier 6's PHPStan-max sweep can proceed against a codebase where every file <300 lines (except gauge_detail.php at 608, which is itself an extraction target — see commit `ad79589`). The grandfather list in phpstan.neon should now be empty or near-empty since most Tier 5 commits dropped baseline entries rather than adding them.
