...
| T-S3 | Cookie `HttpOnly` via `_cookie_params()` (`php/includes/auth.php:_cookie_params`). CSP enforced via nginx snippet (prod-side, not in repo). | ⚠ | Depends on the CSP snippet's `script-src` strictness — confirm in Tier 1 prod check. HSTS not enabled in repo (`deploy/SETUP.md:395` shows "uncomment"; not in `conf/security-headers.conf:7`) — see **[F-1]**. |
...
| T-T3 | `account.php:22-29` whitelists `display_name` only (max 128 chars). `edit.php:47/64` defines `$editable_fields` (per type) and the POST loop only iterates that list (`php/edit.php:92-108`). `propose.php` tier-gates via `$tier = $ed['status']` (`php/includes/propose_handler.php :: _load_propose_context`); needs per-tier whitelist confirmation in Tier 2.3. `review.php` constructs `$applied['reach']` from `$_POST` — the keys must be whitelisted upstream. | ⚠ **[F-7]** | account/edit are clean. propose/review need closer reading to confirm no mass-assignment vectors. |
...
| Threat | Control | Status | Notes |
|---|---|---|---|
| T-I1 | `review.php` calls `require_maintainer()` (the `php/review.php` shim still calls it before delegating to `php/includes/review_handler.php :: handle_review_request`); `propose.php` filters `change_request` by `editor_id = current_editor` (verified in Phase 1 reading). | ✓ | Need Tier 2.2 IDOR sweep to confirm no GET/POST path leaks cross-editor data. |
...
| T-E5 | `safe_next_url()` rejects `^/[^/\\]` (`php/includes/auth_magic_link.php :: safe_next_url`) — `//evil.com`, `/\evil.com`, `///x`, `/\\x` all rejected. Same-origin `/path?…` accepted. | ✓ | Tight regex. Audit gate (`SanityTest.php`) covers it. |
