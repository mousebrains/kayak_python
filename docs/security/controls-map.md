...
| T-S3 | Cookie `HttpOnly` via `_cookie_params()` (`src/kayak/web/php/includes/auth.php:_cookie_params`). CSP enforced via nginx snippet (prod-side, not in repo). | ⚠ | Depends on the CSP snippet's `script-src` strictness — confirm in Tier 1 prod check. HSTS not enabled in repo (`deploy/SETUP.md:395` shows "uncomment"; not in `conf/security-headers.conf:7`) — see **[F-1]**. |
...
| T-S5 | Double-submit cookie + `hash_equals` constant-time compare (`require_csrf()` in `src/kayak/web/php/includes/auth.php:require_csrf`); CSRF cookie rotated on session creation (`set_editor_session()`). Coverage: 10/10 editor POST handlers call `require_csrf()` (verified by grep). | ✓ | Strong. |
...
| T-T3 | `account.php:22-29` whitelists `display_name` only (max 128 chars). `edit.php:47/64` defines `$editable_fields` (per type) and the POST loop only iterates that list (`src/kayak/web/php/edit.php:92-108`). `propose.php` tier-gates via `$tier = $ed['status']` (`src/kayak/web/php/includes/propose_handler.php :: _load_propose_context`); needs per-tier whitelist confirmation in Tier 2.3. `review.php` constructs `$applied['reach']` from `$_POST` — the keys must be whitelisted upstream. | ⚠ **[F-7]** | account/edit are clean. propose/review need closer reading to confirm no mass-assignment vectors. |
| T-T4 | Repo convention is parameterized `prepare(...)->execute([...])`. Grep for `prepare("$|prepare(<concat>"` finds: `IN ($placeholders)` patterns (safe — placeholders constructed from `?, ?, ?` strings); `UPDATE $table SET $sets` patterns at `src/kayak/web/php/edit.php:117` and `src/kayak/web/php/includes/review_logic.php:101`. | ⚠ **[F-8]** | The `UPDATE $table SET $sets` concat is safe given current usage (`$table` ∈ `{reach, gauge}`; `$field` from `$editable_fields` whitelist), but the pattern is a code smell — a future contributor could pass un-whitelisted input. Refactor to const dispatch tables. |
...
| Threat | Control | Status | Notes |
|---|---|---|---|
| T-I1 | `review.php` calls `require_maintainer()` (the `src/kayak/web/php/review.php` shim still calls it before delegating to `src/kayak/web/php/includes/review_handler.php :: handle_review_request`); `propose.php` filters `change_request` by `editor_id = current_editor` (verified in Phase 1 reading). | ✓ | Need Tier 2.2 IDOR sweep to confirm no GET/POST path leaks cross-editor data. |
| T-I2 | `admin.php` requires maintainer (`src/kayak/web/php/admin.php:17`); `account.php` shows only `$ed` (current). Maintainer notification emails (`comment.php`, `propose.php`) include proposer email → goes to operator inbox only. | ✓ | The proposer-email-in-email is by design (Reply-To). |
...
| T-I8 | No entry point renders `edit_history` (verified by grep across `src/kayak/web/php/`). | ✓ | If a maintainer audit view is added later, re-verify. |
...
| T-D3 | Review-list pagination + default `status=pending` filter (verified in `src/kayak/web/php/review.php` reading). | ✓ | UI scales. |
...
| T-E5 | `safe_next_url()` rejects `^/[^/\\]` (`src/kayak/web/php/includes/auth_magic_link.php :: safe_next_url`) — `//evil.com`, `/\evil.com`, `///x`, `/\\x` all rejected. Same-origin `/path?…` accepted. | ✓ | Tight regex. Audit gate (`SanityTest.php`) covers it. |
