...
| 2.2.5 | `review.php` proposal access scope | ✅ | `SELECT * FROM change_request WHERE id = ?` (`php/includes/review_handler.php :: _review_handle_post`). Maintainer is REQUIRED (the `php/review.php` shim calls `require_maintainer()` before delegating to `handle_review_request`), so unrestricted access is the intended model. List view (`php/includes/review_handler.php :: _render_review_list`) joins editor for display; status filter is enum-validated. |
