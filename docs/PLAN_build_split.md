...
1. **Phase 1 — scaffolding + `_shared`.** Create `src/kayak/web/build/{__init__.py,_shared.py}` (`src/kayak/web/__init__.py` already exists). Move the truly shared constants and five small helpers (`_og_meta`, `_load_css`, `_css_link_tag`, `_atomic_write`, `_editor_feature_on`) into `_shared.py`. Re-import them in `cli/build.py` so all current call sites keep working. Each new module gets `logger = logging.getLogger(__name__)` only if it has at least one logging call — adding a logger nothing uses violates CLAUDE.md's "don't add what you don't use." `_shared.py` gets one (used by `_load_css`'s warning); `sparklines.py` does not.
