...
def find_new_usgs_gauges(cache, kayak, active_only=True):
    ...
    known = set(
        r[0] for r in kayak.execute(
            "SELECT usgs_id FROM gauge WHERE usgs_id IS NOT NULL"
        ).fetchall()
    )
...
def find_new_nwps_gauges(cache, kayak):
    ...
    for col in ["nws_id", "nwsli_id", "cbtt_id"]:
        rows = kayak.execute(
            f"SELECT {col} FROM gauge WHERE {col} IS NOT NULL"
        ).fetchall()
        known.update(r[0] for r in rows)

    # Also check source names
    src_names = set(
        r[0] for r in kayak.execute("SELECT name FROM source").fetchall()
    )
    known.update(src_names)

    new = cache.execute(
        "SELECT lid, name, latitude, longitude, state FROM nwps_site"
    ).fetchall()
...
def check_data_status(kayak, days=7):
    ...
    week_ago = (datetime.now(UTC) - timedelta(days=days * 2)).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    """, (week_ago, cutoff_str, cutoff_str)).fetchall()
...
def main():
    ...
    parser.add_argument(
        "--cache-db", type=str, default=str(CACHE_DB),
        help=f"Path to gauge metadata cache (default: {CACHE_DB})",
    )
    parser.add_argument(
        "--kayak-db", type=str, default=str(KAYAK_DB),
    ...
    if usgs_candidates:
        ...
        print("-" * 105)
        for dist, gid, gname, rid, rlabel, has_gauge in usgs_candidates[:30]:
    ...
    if nwps_candidates:
        ...
        print("-" * 105)
        for dist, gid, gname, rid, rlabel, has_gauge in nwps_candidates[:30]:
            print(f"{dist:>4.1f}  {gid:<12} {gname[:45]:<45} {rlabel[:30]:<30} {has_gauge}")
    ...
    if stopped:
        for gid, gname, usgs_id, last_obs, count in stopped:
    ...
    print("=" * 60)
    if started:
        for gid, gname, usgs_id, first_obs, count in started:
    ...
    print("=" * 60)
    if stale:
        for gid, gname, usgs_id, rid, rname, last_obs, dtype in stale:
