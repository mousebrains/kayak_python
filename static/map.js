...
(function () {
  ...
  const DESKTOP_HOVER = window.matchMedia(
  ...
  function fetchOptional(url, label) {
    ...
    return fetch(url)
      .then(function (r) {
        if (!r.ok) {
          console.warn('map: ' + label + ' fetch ' + r.status);
          return null;
        }
        return r.json();
      })
      .catch(function (e) {
        console.warn('map: ' + label + ' fetch failed:', e);
        return null;
      });
  }
  ...
  Promise.all([
    ...
    fetch(stateUrl).then(function (r) {
  ...
  function renderMap(geom, state, gaugesGeom, gaugesState, osmbData) {
    ...
    L.geoJSON(geom, {
      ...
      pointToLayer: function (f, ll) {
        ...
        return L.circleMarker(ll, {
          radius: 6,
          fillColor: COLORS[s] || COLORS.unknown,
          color: '#333',
          weight: 1,
          fillOpacity: 0.8,
        });
      ...
      onEachFeature: function (f, layer) {
        ...
        function buildPopup() {
          ...
          if ('v' in entry) {
            ...
            html +=
              '<div class="rp-reading' + (stale ? ' rp-stale' : '') + '">';
            html += esc(val);
            if (delta)
              html += ' <span class="rp-trend">' + esc(delta) + '</span>';
            html += '</div>';
          }
          html += '<div class="rp-footer">';
          if (ageStr)
            html += '<span class="rp-time">' + esc(ageStr) + '</span>';
          html +=
            '<span class="rp-status-text"><span class="rp-dot" style="color:' +
            dotColor +
            '">&#9679;</span> ' +
            esc(s) +
            '</span>';
          if (classDisplay)
            html += '<span class="rp-tiers">' + esc(classDisplay) + '</span>';
          html += '</div></a>';
          return html;
        }
        ...
        function scheduleClose() {
          ...
          closeTimer = setTimeout(function () {
            closeTimer = null;
            if (!layer._mfHovered && !layer._mfPopupHovered)
              target.closePopup();
        ...
        target.on('mouseout', function () {
          ...
          if (layer._mfCasing)
            layer._mfCasing.setStyle({ weight: REST_CASING.weight });
    ...
    map.on('zoomend', function () {
      ...
      for (const id in layersById) {
        ...
        if (lyr._mfCasing) {
          lyr._mfCasing.setStyle({
            weight: hov ? HOVER_CASING.weight : REST_CASING.weight,
          });
        }
      }
    ...
    if (hasGaugeLayer)
      buildGaugeLayer(gaugesGeom, gaugesState, gaugeLayer, gaugeMarkers);
    if (hasGaugeLayer && showGauges) gaugeLayer.addTo(map);

    function gaugeRadiusForZoom(z) {
      return z == null || z < GAUGE_ZOOM_THRESHOLD
        ? GAUGE_RADIUS_LOW
        : GAUGE_RADIUS_HIGH;
    }
    ...
    function buildGaugeLayer(geom, state, layerGroup, markerArr) {
      ...
      for (let i = 0; i < features.length; i++) {
        ...
        function buildPopup() {
          ...
          if (props.location && props.location !== props.river)
            subtitleParts.push(props.location);
          if (subtitleParts.length) {
            html +=
              '<div class="rp-sub">' +
              esc(subtitleParts.join(' · ')) +
              '</div>';
          }
          const readings = [];
          if (entry.flow)
            readings.push(fmtValue(entry.flow.v, 'flow', entry.flow.u));
          if (entry.gage)
            readings.push(fmtValue(entry.gage.v, 'gage', entry.gage.u));
          if (entry.temperature)
            readings.push(
              fmtValue(entry.temperature.v, 'temperature', entry.temperature.u),
            );
          if (readings.length) {
            html +=
    ...
    function matches(layer) {
      ...
      for (let i = 0; i < tiers.length; i++)
        if (cSet.has(tiers[i])) return true;
      return false;
    }
    ...
    function refilter() {
      ...
      for (let i = 0; i < visible.length; i++)
        if (visible[i]._mfCasing) group.addLayer(visible[i]._mfCasing);
      for (let i = 0; i < visible.length; i++) group.addLayer(visible[i]);
      for (let i = 0; i < visible.length; i++)
        if (visible[i]._mfHit) group.addLayer(visible[i]._mfHit);
      if (countEl)
        countEl.textContent =
          visible.length + ' reach' + (visible.length === 1 ? '' : 'es');
      if (firstPaint) {
        firstPaint = false;
        ...
        if (hasGaugeLayer && showGauges) {
          for (let i = 0; i < gaugeMarkers.length; i++)
            boundsLayers.push(gaugeMarkers[i]);
        }
    ...
    function onBaseChange(name) {
      for (const k in BASE_LAYERS) {
        if (k !== name && map.hasLayer(BASE_LAYERS[k]))
          map.removeLayer(BASE_LAYERS[k]);
      }
  ...
  function addFilterControl(sSet, cSet, onChange, opts) {
    ...
    ctl.onAdd = function () {
      ...
      const t1 = L.DomUtil.create('span', 'mft-line', toggle);
      t1.textContent = 'Layers';
      const t2 = L.DomUtil.create('span', 'mft-line', toggle);
      t2.textContent = 'Filters';
      ...
      if (opts.hasGaugeLayer) {
        ...
        cb.type = 'checkbox';
        cb.checked = opts.showGauges;
        cb.addEventListener('change', function () {
          opts.onOverlayToggle('gauges', cb.checked);
        });
      ...
      STATUSES.forEach(function (s) {
        ...
        cb.type = 'checkbox';
        cb.value = s;
        cb.checked = sSet.has(s);
        cb.addEventListener('change', function () {
          if (cb.checked) sSet.add(s);
          else sSet.delete(s);
          onChange();
        });
        const sw = L.DomUtil.create('span', 'swatch', lab);
        ...
        lab.appendChild(
          document.createTextNode(s.charAt(0).toUpperCase() + s.slice(1)),
        );
      });
      ...
      CLASS_TIERS.forEach(function (t) {
        ...
        cb.type = 'checkbox';
        cb.value = t;
        cb.checked = cSet.has(t);
        cb.addEventListener('change', function () {
          if (cb.checked) cSet.add(t);
          else cSet.delete(t);
          onChange();
        });
      ...
      function updateOverflowHint() {
        panel.classList.toggle(
          'has-overflow',
          panel.scrollHeight - panel.clientHeight > 4,
        );
      }
