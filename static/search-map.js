...
reaches.forEach(function(r){
  ...
  const markerLL=(r.track&&r.track.length>1)?r.track[Math.floor(r.track.length/2)]:[r.lat,r.lon];
  const ic=L.divIcon({className:'',html:'<div style="background:'+c+';color:#fff;padding:2px 6px;border-radius:3px;font:bold 11px sans-serif;white-space:nowrap;cursor:pointer;border:1px solid rgba(0,0,0,.3)">'+esc(r.name||'')+'</div>',iconAnchor:[0,12]});
...
gauges.forEach(function(g){
  const ic=L.divIcon({className:'',html:'<div style="background:#333;color:#ff0;width:14px;height:14px;text-align:center;line-height:14px;font-size:12px;cursor:pointer;border:2px solid #ff0">&#9670;</div>',iconAnchor:[9,9]});
  const m=L.marker([g.lat,g.lon],{icon:ic,zIndexOffset:1000}).addTo(map);
