(function () {
  ...
  function renderSelected() {
    if (!selectedList.length) {
      ol.innerHTML =
        '<li class="empty">No reaches selected yet — tap a checkbox above to add.</li>';
      return;
    }
  ...
  async function loadStates() {
    ...
    if (needed.length) {
      try {
        const url =
          '/picker.php?ajax=1&states=' + encodeURIComponent(needed.join(','));
        const resp = await fetch(url);
        const rows = await resp.json();
        for (const n of needed) {
          const tagged = rows.map((r) =>
            Object.assign({}, r, { state_first: r.state_first || n }),
          );
